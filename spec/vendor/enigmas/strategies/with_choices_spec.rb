...
RSpec.describe Enigmas::Strategies::WithChoice do
  let(:enigma) do
    double('Enigma', question: 'What is the capital of France?', choices: %w[Paris London Berlin], good_result?: true)
  end
  let(:strategy) { described_class.new(enigma) }

  describe '#question' do
    it 'returns the question with choices appended' do
      expect(strategy.question).to eq('What is the capital of France? - one choice - ( Paris / London / Berlin )')
    end
  end
  ...
  describe '#good_result?' do
    context 'when answer is correct' do
      it 'returns true' do
        allow(strategy).to receive(:answer).and_return('Paris')
        allow(enigma).to receive(:good_result?).with('Paris').and_return(true)
    ...
    context 'when answer is incorrect' do
      it 'returns false' do
        allow(strategy).to receive(:answer).and_return('London')
        allow(enigma).to receive(:good_result?).with('London').and_return(false)
